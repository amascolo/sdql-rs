...
pub fn tpch_08(
    ...
    //             acc
    //         });
    let mut c_h = vec![i32::default(); 200001];
    let ptr = c_h.as_mut_ptr() as usize;
    (0..customer.8).into_iter().for_each(move |i| unsafe {
        *(ptr as *mut i32).add(customer.0[i as usize] as usize) = customer.3[i as usize];
    });
...
pub fn tpch_08_parallel(
    ...
    let mut c_h = vec![i32::default(); 200001];
    let ptr = c_h.as_mut_ptr() as usize;
    (0..customer.8).into_par_iter().for_each(move |i| unsafe {
        *(ptr as *mut i32).add(customer.0[i as usize] as usize) = customer.3[i as usize];
    });
    let mut p_h: HashMap<i32, Record<(i32,)>> = (0..part.9)
        .into_par_iter()
