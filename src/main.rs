...
fn q1() -> TypeQ1 {
    let lineitem = load!(
        ...
        ArrayString < 1 >, l_linestatus : ArrayString < 1 >, l_shipdate : Date,
        l_commitdate : Date, l_receiptdate : Date, l_shipinstruct : ArrayString < 25 >,
        l_shipmode : ArrayString < 10 >, l_comment : ArrayString < 44 >
    )("datasets/tpch_datasets/SF_0.01/lineitem.tbl")
    .unwrap();
    let l_h: HashMap<
        Record<(ArrayString<1>, ArrayString<1>)>,
        Record<(
            OrderedFloat<f64>,
            OrderedFloat<f64>,
            OrderedFloat<f64>,
...
// TODO newtype wrapper around ArrayString that implements +=
// type TypeQ5 = HashMap<Record<(ArrayString<25>, OrderedFloat<f64>)>, Bool>;
//
// fn q5() -> TypeQ5 {
//     let customer = load!(
//         c_custkey : i32, c_name : ArrayString < 25 >, c_address : ArrayString < 40 >,
//         c_nationkey : i32, c_phone : ArrayString < 15 >, c_acctbal : OrderedFloat < f64
//         >, c_mktsegment : ArrayString < 10 >, c_comment : ArrayString < 117 >
//     )("datasets/tpch/customer.tbl")
//     .unwrap();
//     let orders = load!(
//         o_orderkey : i32, o_custkey : i32, o_orderstatus : ArrayString < 1 >,
//         o_totalprice : OrderedFloat < f64 >, o_orderdate : Date, o_orderpriority :
//         ArrayString < 15 >, o_clerk : ArrayString < 15 >, o_shippriority : i32, o_comment
//         : ArrayString < 79 >
...
//     .unwrap();
//     let supplier = load!(
//         s_suppkey : i32, s_name : ArrayString < 25 >, s_address : ArrayString < 40 >,
//         s_nationkey : i32, s_phone : ArrayString < 15 >, s_acctbal : OrderedFloat < f64
//         >, s_comment : ArrayString < 101 >
//     )("datasets/tpch/supplier.tbl")
//     .unwrap();
//     let nation = load!(
//         n_nationkey : i32, n_name : ArrayString < 25 >, n_regionkey : i32, n_comment :
//         ArrayString < 152 >
//     )("datasets/tpch/nation.tbl")
//     .unwrap();
//     let region = load!(
//         r_regionkey : i32, r_name : ArrayString < 25 >, r_comment : ArrayString < 152 >
//     )("datasets/tpch/region.tbl")
//     .unwrap();
//     let r_h = (0..region.3)
//         .filter(|&i| region.1[i] == ArrayString::from_str("ASIA").unwrap())
//         .fold(HashMap::new(), |mut acc, i| {
...
//             acc
//         });
//     let c_h: HashMap<i32, Record<(ArrayString<25>, i32)>> = (0..customer.8)
//         .filter(|&i| n_h.contains_key(&customer.3[i]))
//         .fold(HashMap::new(), |mut acc, i| {
//             acc[&customer.0[i]] += Record::new((n_h[&customer.3[i]], customer.3[i]));
//             acc
//         });
//     let o_h: HashMap<i32, Record<(ArrayString<25>, i32)>> = (0..orders.9)
//         .filter(|&i| {
...
//         acc
//     });
//     let l_h: HashMap<ArrayString<25>, OrderedFloat<f64>> = (0..lineitem.16)
//         .filter(|&i| {
...
// TODO newtype wrapper around ArrayString that implements +=
// type TypeQ9 = HashMap<Record<(ArrayString<25>, i32, OrderedFloat<f64>)>, Bool>;
//
// fn q9() -> TypeQ9 {
//     let part = load!(
//         p_partkey : i32, p_name : ArrayString < 55 >, p_mfgr : ArrayString < 25 >,
//         p_brand : ArrayString < 10 >, p_type : ArrayString < 25 >, p_size : i32,
//         p_container : ArrayString < 10 >, p_retailprice : OrderedFloat < f64 >, p_comment
//         : ArrayString < 23 >
...
//         ps_supplycost : OrderedFloat < f64 >, ps_comment : ArrayString < 199 >
//     )("datasets/tpch/partsupp.tbl")
//     .unwrap();
//     let orders = load!(
//         o_orderkey : i32, o_custkey : i32, o_orderstatus : ArrayString < 1 >,
//         o_totalprice : OrderedFloat < f64 >, o_orderdate : Date, o_orderpriority :
...
//     )("datasets/tpch/nation.tbl")
//     .unwrap();
//     let n_h: HashMap<i32, Record<(ArrayString<25>,)>> =
...
//             acc
//         });
//     let o_h = (0..orders.9).fold(Vec::new(), |mut acc, i| {
//         acc[orders.0[i].try_into().unwrap()] += orders.4[i];
//         acc
//     });
//     let l_h: HashMap<Record<(ArrayString<25>, i32)>, Record<(OrderedFloat<f64>,)>> = (0..lineitem
//         .16)
//         .filter(|&i| ps_h.contains_key(&Record::new((lineitem.1[i], lineitem.2[i]))))
//         .fold(HashMap::new(), |mut acc, i| {
//             acc[&Record::new((
//                 ps_h[&Record::new((lineitem.1[i], lineitem.2[i]))].0,
//                 o_h[lineitem.0[i].try_into().unwrap()],
//             ))] += Record::new((lineitem.5[i] * (OrderedFloat(1f64) - lineitem.6[i])
...
//             acc
//         });
//     let custkey_to_name: HashMap<i32, Record<(ArrayString<25>,)>> =
...
//         Record<(ArrayString<25>, i32, i32, i32, OrderedFloat<f64>)>,
